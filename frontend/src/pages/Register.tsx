...
export default function Register() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      setServerError(readError(err, "Couldn't create your account."));
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-6 bg-canvas">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted mt-1">Start tailoring resumes in minutes.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-surface border border-line rounded-xl shadow-card p-6 space-y-4">
          <div>
            ...
            />
            {errors.full_name && <p className="text-xs text-bad mt-1">{errors.full_name.message}</p>}
          </div>
          <div>
